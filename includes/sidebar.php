...
if (isset($_GET['userlevel'])) {
    $_SESSION['selected_role'] = $_GET['userlevel'];
    // Redirect to remove the userlevel parameter from URL
...
// Determine path to dashboard based on current directory
$current_dir = basename(dirname($_SERVER['PHP_SELF']));
$dashboard_path = ($current_dir === 'dashboard') ? './' : '../dashboard/';
...
<aside class="w-64 bg-white border-r border-gray-200 flex flex-col fixed lg:relative h-screen lg:h-auto lg:translate-x-0 transform -translate-x-full transition-transform duration-300 z-50" id="sidebar">
    ...
    <div class="p-6 border-b border-gray-200">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="<?php echo $dashboard_path; ?>?userlevel=admin" class="px-3 py-1.5 text-xs font-medium rounded-lg transition <?php echo ($current_role === 'admin') ? 'bg-[#003047] text-white hover:bg-[#004d6b]' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'; ?>">
                Admin
            </a>
            <a href="<?php echo $dashboard_path; ?>?userlevel=receptionist" class="px-3 py-1.5 text-xs font-medium rounded-lg transition <?php echo ($current_role === 'receptionist') ? 'bg-[#003047] text-white hover:bg-[#004d6b]' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'; ?>">
                Receptionist
            </a>
            <a href="<?php echo $dashboard_path; ?>?userlevel=technician" class="px-3 py-1.5 text-xs font-medium rounded-lg transition <?php echo ($current_role === 'technician') ? 'bg-[#003047] text-white hover:bg-[#004d6b]' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'; ?>">
                Technician
            </a>
        </div>
    </div>
    ...
    <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
        ...
        <?php if (isMenuAccessible('waiting_list', $current_role, $menu_items)): ?>
        <a href="../booking/waiting-list.php" class="flex items-center gap-3 px-4 py-3 rounded-lg font-medium <?php echo (basename($_SERVER['PHP_SELF']) == 'waiting-list.php' && basename(dirname($_SERVER['PHP_SELF'])) == 'booking') ? 'bg-[#e6f0f3] text-[#003047]' : 'text-gray-700 hover:bg-gray-50'; ?> transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a href="../booking/tickets.php" class="flex items-center gap-3 px-4 py-3 rounded-lg font-medium <?php echo (basename($_SERVER['PHP_SELF']) == 'tickets.php' && basename(dirname($_SERVER['PHP_SELF'])) == 'booking') ? 'bg-[#e6f0f3] text-[#003047]' : 'text-gray-700 hover:bg-gray-50'; ?> transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <?php if (isMenuAccessible('turn_tracker', $current_role, $menu_items)): ?>
        <a href="../turn-tracker/" class="flex items-center gap-3 px-4 py-3 text-gray-700 hover:bg-gray-50 rounded-lg transition <?php echo (basename(dirname($_SERVER['PHP_SELF'])) == 'turn-tracker') ? 'bg-[#e6f0f3] text-[#003047]' : ''; ?>">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <?php if (isMenuAccessible('payout', $current_role, $menu_items)): ?>
        <a href="../payout/" class="flex items-center gap-3 px-4 py-3 text-gray-700 hover:bg-gray-50 rounded-lg transition <?php echo (basename(dirname($_SERVER['PHP_SELF'])) == 'payout') ? 'bg-[#e6f0f3] text-[#003047]' : ''; ?>">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
