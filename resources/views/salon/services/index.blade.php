...
function renderListView() {
    ...
    var list = getPaginatedServices();
    if (list.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="px-6 py-12 text-center text-gray-500 text-sm">No services found.</td></tr>';
        return;
    }
    list.forEach(function(s, i) {
        var color = colorClasses[i % colorClasses.length];
        ...
        var colorDot = s.color ? '<span class="inline-block w-3 h-3 rounded-full flex-shrink-0" style="background:' + s.color + '"></span>' : '<span class="inline-block w-3 h-3 rounded-full flex-shrink-0 border-2 border-gray-300"></span>';
        row.innerHTML = '<td class="px-6 py-4 whitespace-nowrap"><div onclick="openServiceModal(' + JSON.stringify(s).replace(/"/g, '&quot;') + ')" class="flex items-center cursor-pointer">' + thumbCell + '<div class="text-sm font-medium text-gray-900 flex items-center gap-2">' + colorDot + (s.name || '') + '</div></div></td><td class="px-6 py-4"><div onclick="openServiceModal(' + JSON.stringify(s).replace(/"/g, '&quot;') + ')" class="text-sm text-gray-900 cursor-pointer">' + (s.description || '') + '</div></td><td class="px-6 py-4 whitespace-nowrap"><div onclick="openServiceModal(' + JSON.stringify(s).replace(/"/g, '&quot;') + ')" class="text-sm font-bold text-gray-900 cursor-pointer">' + window.salonFormatMoney(parseFloat(s.price || 0)) + '</div></td><td class="px-6 py-4 whitespace-nowrap"><div onclick="openServiceModal(' + JSON.stringify(s).replace(/"/g, '&quot;') + ')" class="text-sm text-gray-900 cursor-pointer">' + (s.service_count != null ? s.service_count : 0) + '</div></td><td class="px-6 py-4 whitespace-nowrap"><div onclick="openServiceModal(' + JSON.stringify(s).replace(/"/g, '&quot;') + ')" class="cursor-pointer">' + statusHtml + '</div></td><td class="px-6 py-4 whitespace-nowrap text-sm text-right"><div class="flex items-center justify-end gap-2"><button type="button" onclick="event.stopPropagation(); openEditServiceModal(' + s.id + ')" class="inline-flex items-center justify-center w-8 h-8 cursor-pointer bg-[#003047] text-white rounded-lg hover:bg-[#002535] transition active:scale-95" title="Quick Edit"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></button><button type="button" onclick="event.stopPropagation(); deleteService(' + s.id + ', \'' + (s.name || '').replace(/'/g, "\\'") + '\')" class="inline-flex items-center justify-center w-8 h-8 cursor-pointer text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition active:scale-95" title="Delete"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></div></td>';
...
function buildServiceFormData(form) {
    ...
    var serviceCountEl = form.querySelector('select[name="service_count"]');
    fd.append('service_count', serviceCountEl ? (parseFloat(serviceCountEl.value) || 0) : 0);
