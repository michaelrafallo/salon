...
function renderGrid() {
    ...
    el.innerHTML = list.map(function(t, i) {
        ...
        var stats = isAdmin ? '<div class="grid grid-cols-3 gap-3 mb-4 pt-4 border-t border-gray-200"><div><p class="text-xs text-gray-500">Total</p><p class="text-lg font-bold text-gray-900">' + window.salonFormatMoney(t.totalEarnings || 0) + '</p></div><div><p class="text-xs text-gray-500">Tip</p><p class="text-lg font-bold text-gray-900">' + window.salonFormatMoney(t.totalTips || 0) + '</p></div><div><p class="text-xs text-gray-500">Commission</p><p class="text-lg font-bold text-gray-900">' + window.salonFormatMoney(t.totalCommission || 0) + '</p></div></div>' : '';
        var clockButtonClass = clock.isLoggedIn ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600';
        var clockButtonText = clock.isLoggedIn ? 'Clock Out' : 'Clock In';
        return '<div class="technician-card bg-white rounded-lg shadow-sm border border-gray-200 p-5 hover:shadow-md transition-shadow flex flex-col" data-status="' + statusText.toLowerCase() + '"><div onclick="window.location.href=\'' + viewUrl + '?id=' + t.id + '\'" class="cursor-pointer flex-1"><div class="flex items-center gap-4 mb-4">' + getTechAvatar(t, 'w-16 h-16', 'text-2xl', color) + '<div class="flex-1 min-w-0"><h3 class="font-semibold text-gray-900 text-lg truncate">' + name + '</h3><p class="text-sm text-gray-500">Technician</p><div class="flex items-center gap-2 mt-1"><span id="status-badge-' + t.id + '" class="px-2 py-1 ' + statusClass + ' text-xs font-medium rounded">' + statusText + '</span></div></div></div>' + stats + '</div><div class="flex gap-2 pt-4 mt-4 border-t border-gray-100" onclick="event.stopPropagation()"><button type="button" id="gridLoginToggle-' + t.id + '" onclick="salonTechToggleLogin(' + t.id + ')" class="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2 ' + clockButtonClass + ' text-white rounded-lg transition font-medium text-sm active:scale-95"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>' + clockButtonText + '</button><button type="button" onclick="salonTechPrintTicket(' + t.id + ')" class="inline-flex items-center justify-center px-4 py-2 text-white bg-[#003047] rounded-lg hover:bg-[#002535] transition font-medium text-sm active:scale-95"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg></button></div></div>';
    }).join('');
}
function renderList() {
    ...
    tbody.innerHTML = list.map(function(t, i) {
        ...
        var adminCols = isAdmin ? '<td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">' + window.salonFormatMoney(t.totalEarnings || 0) + '</td><td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">' + window.salonFormatMoney(t.totalTips || 0) + '</td><td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">' + window.salonFormatMoney(t.totalCommission || 0) + '</td>' : '';
        return '<tr onclick="event.stopPropagation(); window.location.href=\'' + viewUrl + '?id=' + t.id + '\'" class="technician-row hover:bg-gray-50 cursor-pointer transition" data-status="' + statusText.toLowerCase() + '"><td class="px-6 py-4 whitespace-nowrap"><div class="flex items-center">' + getTechAvatar(t, 'w-10 h-10 mr-3', 'text-sm', color) + '<div class="text-sm font-medium text-gray-900">' + name + '</div></div></td><td class="px-6 py-4 whitespace-nowrap"><span id="status-badge-list-' + t.id + '" class="px-2 py-1 ' + statusClass + ' text-xs font-medium rounded">' + statusText + '</span></td>' + adminCols + '<td class="px-6 py-4 whitespace-nowrap" onclick="event.stopPropagation()"><div class="space-y-1" id="clockInfo-' + t.id + '"><div class="text-xs text-gray-900 font-medium" id="clockTimeIn-' + t.id + '">In: ' + clockInTime + '</div><div class="text-xs text-gray-900 font-medium" id="clockTimeOut-' + t.id + '">Out: ' + clockOutTime + '</div></div></td><td class="px-6 py-4 whitespace-nowrap text-right" onclick="event.stopPropagation()"><div class="flex items-center justify-end gap-2"><button id="loginToggle-' + t.id + '" onclick="salonTechToggleLogin(' + t.id + ')" class="w-20 text-center px-3 py-1.5 ' + buttonClass + ' text-white text-xs font-medium rounded hover:opacity-90 transition active:scale-95">' + buttonText + '</button><button onclick="salonTechPrintTicket(' + t.id + ')" class="px-3 py-1.5 bg-[#003047] text-white text-xs font-medium rounded hover:bg-[#002535] transition active:scale-95 flex items-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>Ticket</button></div></td></tr>';
    }).join('');
}
