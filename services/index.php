...
<main class="flex-1 overflow-y-auto bg-gray-50 lg:ml-0 pt-16 lg:pt-0">
    <div class="p-4 sm:p-6 lg:p-8">
        ...
        <div class="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div id="resultsCounter" class="text-sm text-gray-600">
...
async function fetchServices() {
    try {
        ...
        const data = await response.json();
        allServices = data.services.filter(service => service.active);
        servicesData = allServices; // Initially show all services
        applyFilters();
    } catch (error) {
...
function renderGridView() {
    ...
    if (paginatedServices.length === 0) {
        gridView.innerHTML = `
            <div class="col-span-full bg-white border border-dashed border-gray-300 rounded-lg p-10 text-center">
...
function getPaginatedServices() {
    ...
    return servicesData.slice(startIndex, startIndex + PAGE_SIZE);
}
...
function renderPagination() {
    ...
    if (PAGE_SIZE === 'all' || PAGE_SIZE === Infinity || servicesData.length <= PAGE_SIZE || totalPages <= 1) {
...
function filterByCategory(category) {
    ...
    // Update dropdown to reflect the selected category
    const categoryFilter = document.getElementById('categoryFilter');
    if (categoryFilter) {
...
function applyFilters() {
    servicesData = allServices.filter(service => {
        ...
        if (currentCategoryFilter !== '') {
            categoryMatch = service.categories && service.categories.includes(currentCategoryFilter);
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const savedPerPage = localStorage.getItem('servicesPerPage');
    if (savedPerPage) {
