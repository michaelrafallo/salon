        ...
        <div id="step2Content" class="step-content hidden flex-1">
            <div class="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-6 items-stretch">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mt-6">
                        ...
                        <div class="flex border-b border-gray-200 mb-4">
                            ...
                            <button type="button" id="tipEvenTab" onclick="switchTipSplitMode('even')" class="flex-1 px-4 py-2 text-sm font-medium text-gray-500 border-b-2 border-transparent hover:text-gray-700 transition">
                                Even
                            </button>
                        ...
                        <div id="evenTabContent" class="hidden">
                            ...
                            <div class="mb-6">
                                ...
                                <div>
                                    <input type="number" name="tip" id="tipInputEven" step="0.01" min="0" value="0" oninput="updateTipFromInput()" placeholder="0.00" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#003047] focus:border-transparent" />
                                </div>
                            </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 flex flex-col space-y-4 lg:pl-6 h-full">
                    ...
                    <!-- Pay Now Button -->
                    <form onsubmit="processPayment(event)" class="mt-auto">
...
function addServiceToCart(serviceName, servicePrice) {
    ...
    // If on step 2, recalculate tip percentages based on service amounts
...
function renderCheckoutStep() {
    ...
    setTimeout(() => {
        ...
        // Ensure cash content is visible by default
...
function setTipPercentage(percentage) {
    ...
    const tipInputEven = document.getElementById('tipInputEven');
...
function updateTipFromInput() {
    const tipInput = document.getElementById('tipInput');
    const tipInputEven = document.getElementById('tipInputEven');
    const tipValue = parseFloat(tipInput ? tipInput.value : (tipInputEven ? tipInputEven.value : 0)) || 0;
...
function updatePaymentTotal() {
    ...
    // Update Amount display if card payment is selected
...
function switchTipSplitMode(mode) {
    ...
    if (mode === 'percentage') {
        ...
        // Automatically apply even split when switching to even mode
        splitTipEvenly();
    } else if (mode === 'custom') {
        if (customTab) {
...
// Calculate technician service totals from cart
function calculateTechnicianServiceTotals() {
    const technicianTotals = {};
    let totalServices = 0;
    
    // Calculate total for each technician
    cart.forEach(item => {
        const technicianIdStr = item.technician_id.toString();
        const itemTotal = item.price * item.quantity;
        
        if (!technicianTotals[technicianIdStr]) {
            technicianTotals[technicianIdStr] = 0;
        }
        technicianTotals[technicianIdStr] += itemTotal;
        totalServices += itemTotal;
    });
    
    return { technicianTotals, totalServices };
}
...
function renderTechniciansTipSplit() {
    if (!assignedTechnicianIds || assignedTechnicianIds.length === 0) {
        ...
        const tipSectionEven = document.getElementById('techniciansTipSectionEven');
        const tipSectionCustom = document.getElementById('techniciansTipSectionCustom');
        if (tipSection) tipSection.classList.add('hidden');
        if (tipSectionEven) tipSectionEven.classList.add('hidden');
    ...
    // Calculate technician service totals
    const { technicianTotals, totalServices } = calculateTechnicianServiceTotals();
    
    // Calculate percentages based on service amounts for Percentage tab
    if (tipSplitMode === 'percentage' && totalServices > 0) {
        assignedTechnicians.forEach(technician => {
            ...
            if (technicianServiceTotal > 0) {
                const percentage = Math.round(((technicianServiceTotal / totalServices) * 100) * 10) / 10; // Round to 1 decimal place
                const amount = (paymentTip * percentage) / 100;
        ...
        // For Even tab, calculate even split only for technicians with services
        const techniciansWithServices = assignedTechnicians.filter(technician => {
            const technicianIdStr = technician.id.toString();
            return (technicianTotals[technicianIdStr] || 0) > 0;
        });
        
        if (techniciansWithServices.length > 0) {
            const evenPercentage = Math.round((100 / techniciansWithServices.length) * 10) / 10; // Round to 1 decimal place
            const evenAmount = paymentTip / techniciansWithServices.length;
            ...
            // No technicians with services, set all to 0
            assignedTechnicians.forEach(technician => {
                const technicianIdStr = technician.id.toString();
                technicianTips[technicianIdStr] = {
                    percentage: 0,
    ...
    const tipListHTML = assignedTechnicians.map(technician => {
        ...
        // Get technician service total for display
        const technicianServiceTotal = technicianTotals[technicianIdStr] || 0;
        const hasServices = technicianServiceTotal > 0;
        
        // For technicians without services, set percentage and amount to 0
        let tipData, currentPercentage, calculatedAmount;
        if (!hasServices) {
            currentPercentage = 0;
            calculatedAmount = 0;
        } else {
        ...
        // Apply muted/disabled styling if no services
        const cardClasses = hasServices 
            ? "flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200" 
            : "flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200 opacity-50";
        const nameClasses = hasServices 
            ? "text-sm font-medium text-gray-900 truncate" 
            : "text-sm font-medium text-gray-400 truncate";
        const photoClasses = hasServices 
            ? "w-10 h-10 rounded-full object-cover border-2 border-white" 
            : "w-10 h-10 rounded-full object-cover border-2 border-gray-300 opacity-60";
        const initialsBgClasses = hasServices 
            ? "w-10 h-10 bg-[#e6f0f3] rounded-full flex items-center justify-center border-2 border-white" 
            : "w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center border-2 border-gray-300";
        const initialsTextClasses = hasServices 
            ? "text-sm font-bold text-[#003047]" 
            : "text-sm font-bold text-gray-400";
        
        return `
            <div class="${cardClasses}" data-technician-id="${technicianIdStr}">
                <div class="flex-shrink-0">
                    ${profilePhoto 
                        ? `<img src="${profilePhoto}" alt="${fullName}" class="${photoClasses}">`
                        : `<div class="${initialsBgClasses}">
                            <span class="${initialsTextClasses}">${initials}</span>
                        </div>`
                    }
                </div>
                <div class="flex-1 min-w-0">
                ...
                ${tipSplitMode === 'percentage' || tipSplitMode === 'even' ? `
                <div class="flex gap-2 flex-1">
                    <div class="flex-1">
                        <div class="flex items-center border ${hasServices ? 'border-gray-300' : 'border-gray-200'} rounded-lg">
                            <input type="number" 
                                 id="tip-percentage-${technicianIdStr}" 
                                 value="${currentPercentage % 1 === 0 ? Math.round(currentPercentage) : currentPercentage.toFixed(1)}" 
                                 step="0.1" 
                                 min="0" 
                                 max="100" 
                                 ${!hasServices || tipSplitMode === 'percentage' || tipSplitMode === 'even' ? 'readonly tabindex="-1"' : ''}
                                 ${!hasServices ? 'disabled' : ''}
                                 ${tipSplitMode === 'percentage' || tipSplitMode === 'even' ? '' : 'oninput="updateTechnicianTipPercentage(\'' + technicianIdStr + '\', this.value)"'}
                                 class="flex-1 px-3 py-2 text-sm ${!hasServices || tipSplitMode === 'percentage' || tipSplitMode === 'even' ? 'bg-gray-100 cursor-not-allowed' : ''} ${!hasServices ? 'text-gray-400' : 'text-gray-900'} focus:outline-none focus:ring-2 focus:ring-[#003047] focus:border-transparent rounded-l-lg border-0">
                            <span class="px-3 py-2 text-sm ${hasServices ? 'text-gray-500' : 'text-gray-300'} bg-gray-50 border-l ${hasServices ? 'border-gray-300' : 'border-gray-200'} rounded-r-lg">%</span>
                        </div>
                    </div>
                    <div class="flex-1">
                        <div class="flex items-center border ${hasServices ? 'border-gray-300' : 'border-gray-200'} rounded-lg">
                            <input type="number" 
                                 ...
                                 step="0.01" 
                                 min="0" 
                                 readonly tabindex="-1" ${!hasServices ? 'disabled' : ''}
                                 class="flex-1 px-3 py-2 text-sm bg-gray-100 cursor-not-allowed ${!hasServices ? 'text-gray-400' : 'text-gray-900'} focus:outline-none focus:ring-2 focus:ring-[#003047] focus:border-transparent rounded-l-lg border-0">
                            <span class="px-3 py-2 text-sm ${hasServices ? 'text-gray-500' : 'text-gray-300'} bg-gray-50 border-l ${hasServices ? 'border-gray-300' : 'border-gray-200'} rounded-r-lg">$</span>
                        </div>
                    </div>
                </div>
                ...
                <div>
                        <div class="flex items-center border ${hasServices ? 'border-gray-300' : 'border-gray-200'} rounded-lg">
                            <input type="number" 
                                 ...
                                 step="0.01" 
                                 min="0" 
                                 ${!hasServices ? 'readonly tabindex="-1" disabled' : ''}
                                 ${hasServices ? 'oninput="updateTechnicianTipAmount(\'' + technicianIdStr + '\', this.value)"' : ''}
                                 class="flex-1 px-3 py-2 text-sm ${!hasServices ? 'bg-gray-100 cursor-not-allowed text-gray-400' : ''} focus:outline-none focus:ring-2 focus:ring-[#003047] focus:border-transparent rounded-l-lg border-0">
                            <span class="px-3 py-2 text-sm ${hasServices ? 'text-gray-500' : 'text-gray-300'} bg-gray-50 border-l ${hasServices ? 'border-gray-300' : 'border-gray-200'} rounded-r-lg">$</span>
                        </div>
                    </div>
                `}
...
function updateTotalTipSplit() {
    ...
    const totalTipSplitDisplayEven = document.getElementById('totalTipSplitDisplayEven');
    ...
    if (totalTipSplitDisplayEven) {
        totalTipSplitDisplayEven.textContent = `$${totalSplit.toFixed(2)}`;
    }
...
function selectPaymentMethod(method) {
    ...
    if (method === 'cash') {
        ...
        // Auto-fill amount display with total from Transaction Details
        const checkoutTotalDisplay = document.getElementById('checkoutTotalDisplay');
        if (checkoutTotalDisplay) {
            const totalText = checkoutTotalDisplay.textContent.replace('$', '').trim();
